...
class CTGANSynthesizer(BaseSynthesizer):
    ...
    def fit(self, data,data_info):
        ...
        data_sampler = Sampler(data, data_info)

        data_dim = data.shape[1]
        self.cond_generator = Cond(data, data_info)
        ...
        for i in range(self.epochs):
            for id_ in range(steps_per_epoch):
                ...
                fakeact = apply_activate(fake, data_info)
                ...
                if condvec is None:
                    ...
                    cross_entropy = cond_loss(fake, data_info, c1, m1)
...
def train_ctgan(data,cat_vars,seed,epochs,bs):
    data_info = get_data_info(data,cat_vars)
    tf = BaseTransformer(data,data_info)
    datainput= tf.transform()
    vae = CTGANSynthesizer(epochs = epochs,seed=seed,batch_size = bs)
    vae.fit(datainput,data_info)
    return vae, tf 